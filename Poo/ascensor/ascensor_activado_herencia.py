...
class Ascensor(Elevador):
	...
	def checkRep(self): 												# +1
		if 0 <= self.personas <= self.capacidad:
			raise Exception # especificar el tipo de excepcion
		elif 0 <= self.peso   <= self.pesoMaximo:
			raise Exception
		elif self.piso in self.plantas:
			raise Exception
		else: pass
	...
	def bajaPersona(self, numeroPersonas): # completar esto en funcion de la excepcion recibida
		self.personas -= numeroPersonas
		try:
			self.checkRep()
		except:  # terminar esto: lanzar mensaje en funcion del tipo de excepcion que recibe
			self.personas = 0
...
if __name__ == '__main__':
	...
	ascensor.cambiarPlanta(0)
	ascensor.subePersona(3)
	print(ascensor)
	ascensor.cambiarPlanta(4)
	ascensor.bajaPersona(2)
	print(ascensor)
	ascensor.cambiarPlanta(5)
